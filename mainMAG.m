...
global a v r rpy Cn2b ba bg bm dr dv fs fsc ws wsc epsilon WASPloc timestamps_IMU...
...
if nargin < 3, loss_period = 10; end  % How long between signal loss events 10
if nargin < 2, loss_length = 2; end  % Length of signal loss 2
if nargin < 1
...
[wsc, fsc, fsoc, msc] = deal(zeros(3,nS)); % Bias corrected gyro and accel in s-frame
[ba, bg, bm, epsilon, dv, dr, actual_ba, actual_bg] = deal(zeros(3,nS_filter+1)); % filter error estimates
[innov, Sk] = deal(zeros(9,nS_filter+1));       % filter residuals + std dev
[Pk] = deal(zeros(18,nS_filter+1));             % filter std dev
...
for t=1:nS  % timestep for the INS
    ...
    msc(:,t) = ms(:,t) - bm(:,tf);    % bias corrected accel in s-frame
    ...
    if( tf <= numel(timestamps_WASP) && t < numel(timestamps_IMU) && timestamps_WASP(tf) < timestamps_IMU(t+1) )  
        ...
        [ ba_error, bg_error, bm_error, dv(:,tf+1), dr(:,tf+1), epsilon(:,tf+1), innov(:,tf+1), Sk(:,tf+1), Pk(:,tf+1) ] ...
            = errorfilter( Cs2b, Cb2n, fsoc(:,t), msc(:,t), dt_filter, dr_wasp, mn_cal, gn_cal );

        ba(:,tf+1) = ba(:,tf) + ba_error;
        bg(:,tf+1) = bg(:,tf) + bg_error;
        bm(:,tf+1) = bm(:,tf) + bm_error;
