...
Pinitial = diag([2; 2; 2; 2; 2; 2; 0.1; 0.1; 0.1; 0.5; 0.5; 0.5; 10; 10; 10;]);
...
Na = 50;    % Accel sensor noise cov (in mg squared) 50 % increase to put less weight on observation
Ng = 2;   % Gyro sensor noise cov (in deg/s squared) 2 % increase to make bias estimate more stable

Ua = 30;  % Accel bias drift noise cov 30
Ug = 0.01;   % Gyro bias drift noise cov 0.01 % decreas to make bias estimate stable

Na_obs =  0.00005;  % Noise in each component of the accel and mag direction vector 0.0005 0.00005
...
alpha_f = 100;            % Accel observation 20 100
alpha_m = 50;          % Magnetometer observation 30 100
...
%   Measurement
zk = [ dr_wasp;     fs_dir - Cn2s*gn_dir;      ms_dir - Cn2s*mn_cal_dir];
...
Hk = [  eye(3),     zeros(3,3),     zeros(3,3),                zeros(3,3),     zeros(3,3);
       ...
       zeros(3,3), zeros(3,3),     Cn2s*mat_cross(mn_cal_dir),zeros(3,3),     zeros(3,3)];
...
%innov = [innov; zeros(6,1)];
%Sk_diag = [Sk_diag; zeros(6,1)];
