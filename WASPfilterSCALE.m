function [ sa, ba, bg, dv, dr, epsilon, innov, Sk_diag, Pk_diag ]...
    = WASPfilter( Cs2b, Cb2n, fs_mean, dt, dr_wasp )
%errorfilter Implements an error state filter
%   outputs of the function are:
%   sa - estimated accelerometer scale factor error in the s-frame
...
Pinitial = diag([2; 2; 2; 2; 2; 2; 0.01; 0.01; 0.01; 1; 1; 1; 100; 100; 100; 0.02; 0.02; 0.02]);
...
Ua = 0;  % Accel bias drift noise cov 30
Ug = 0;   % Gyro bias drift noise cov 0.01 % decreas to make bias estimate stable

Sa = 0;  % Accel scale factor drift noise cov 0
...
%   position error, velocity error, orientation error, bias gyro, bias
%   accel, scale accel

xkm = zeros(18,1); 

%   State transition matrix     
Ft = [  zeros(3,3), eye(3),     zeros(3,3),     zeros(3,3),        zeros(3,3),      zeros(3,3);
        zeros(3,3), zeros(3,3), fn_cross,       zeros(3,3),        mg2ms(Cs2n),     mg2ms(Cs2n*diag(fs_mean));
        zeros(3,3), zeros(3,3), zeros(3,3),     deg2rad(-Cs2n),    zeros(3,3),      zeros(3,3);
...
Qt = diag([Na*ones(3,1); Ng*ones(3,1); Ug*ones(3,1); Ua*ones(3,1); Sa*ones(3,1)]);

G = [  zeros(3,3),          zeros(3,3),     zeros(3,3),      zeros(3,3),    zeros(3,3);
       mg2ms(Cs2n),         zeros(3,3),     zeros(3,3),      zeros(3,3),    zeros(3,3);
       zeros(3,3),          deg2rad(-Cs2n), zeros(3,3),      zeros(3,3),    zeros(3,3);
...
%   Measurement matrix
Hk = [ eye(3),     zeros(3,3),     zeros(3,3),    zeros(3,3),    zeros(3,3),     zeros(3,3)];   
