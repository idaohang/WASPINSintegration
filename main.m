...
if nargin < 3, loss_period = 15; end  % How long between signal loss events 10
if nargin < 2, loss_length = 5; end  % Length of signal loss 2
if nargin < 1
...
for t=1:nS  % timestep for the INS
    ...
    if( tf <= numel(timestamps_WASP) && t < numel(timestamps_IMU) && timestamps_WASP(tf) < timestamps_IMU(t+1) )  
        ...
        [ ba_error, bg_error, dv(:,tf+1), dr(:,tf+1), epsilon(:,tf+1), innov(:,tf+1), Sk(:,tf+1), Pk(:,tf+1) ] ...
            = errorfilter( Cs2b, Cb2n, fsoc(:,t), ms(:,t), dt_filter, dr_wasp, mn_cal, gn_cal/1000 );
